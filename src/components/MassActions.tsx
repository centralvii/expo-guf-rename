          ...
          {activeVars.map((variable) => (
            <div key={variable.key} className="mass-field">
              <div className="mass-field__header">
                <label className="mass-field__label">{variable.label}</label>
                <span className="mass-field__tag">{`{${variable.key}}`}</span>
              </div>
              ...
              <div className="mass-field__input-wrap">
                <Input
                  ...
                  placeholder={`Значение для {${variable.key}}`}
                  fullWidth
                  noContainer
                />
                <IconButton
        ...
        <div className="mass-actions__inactive">
          ...
          <div className="mass-actions__inactive-tags">
            {inactiveVars.map((variable) => (
              <TagChip
                key={variable.key}
                className="mass-actions__inactive-tag"
                mono
                removable
                removeLabel="Удалить переменную"
                onRemove={() => onRemoveVariable(variable.key)}
              >
                {`{${variable.key}}`}
              </TagChip>
            ))}
        ...
        <label className="start-number-label">
          ...
          <Input
            ...
            onChange={(event) => {
              ...
              onStartNumberChange(Number.isFinite(value) ? value : 1);
            }}
            noContainer
